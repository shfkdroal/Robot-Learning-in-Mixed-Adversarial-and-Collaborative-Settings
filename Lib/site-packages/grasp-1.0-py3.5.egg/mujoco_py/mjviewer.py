...
class MjViewerBasic(cymj.MjRenderContextWindow):
    ...
    def __init__(self, sim):
        ...
        self._needselect=0
        self.tans = self.make_unit_disk(18)
    ...
    # debug
    def make_unit_disk(self, N):
        "Returns N complex numbers on the unit circle (roots of unity)."
        points = [exp(2j * pi * i / N) for i in range(N)]
        tans = [pt.imag/pt.real for pt in points]
        return tans

    def closest_idx(self, t, tans):
        dis = [abs(degrees(t) - degrees(tan)) for tan in tans]
    ...
    def _cursor_pos_callback(self, window, xpos, ypos):
        ...
        with self._gui_lock:
            if self.pert.active:
                ...
                idx = self.closest_idx(dy/(dx+1e-9), self.tans)
                # self.move_perturb(action, dx/height, self.tans[idx]*dx/height)
                print('tans[idx] ', self.tans[idx])
                print('odx/dx: {}/{}, ody/dy: {}/{} '.format(dx/height, dy/height, dx/height, self.tans[idx]*dx/height))
