...
class model:
    def __init__(self):
        self.BATCH_SIZE = 128
        self.SHAKE_SIZE = 6
    def initial_weights(self, weight_file='/home/baxter/ros_ws/src/adversarial_grasping/learner/grasper/models/weights/bvlc_alexnet.npy'):
        ...
        if weight_file:
            ...
            fc8W_init = tf.truncated_normal([1024,self.SHAKE_SIZE], stddev=    0.01)
            fc8b_init = tf.constant(0.1, shape=[self.SHAKE_SIZE]) 
        else:
            ...
            fc7b_init = tf.constant(0.1, shape=[1024]) 
            # fc8W_init = tf.truncated_normal([1024,self.SHAKE_SIZE], stddev=    0.1)
            # fc8b_init = tf.constant(0.1, shape=[self.SHAKE_SIZE]) 
    ...
    def gen_loss(self, fc8, shake_label_batch, shake_success_batch):
        fc8_shape = tf.shape(fc8)
        input_batch_size = tf.gather(fc8_shape, 0) # computiong batch size as an op from the output
        shake_one_hot = tf.one_hot(shake_label_batch, depth=self.SHAKE_SIZE, on_value=1.0, off_value=0.0)
        shake_acted = tf.reduce_sum(fc8 * shake_one_hot, reduction_indices=1, name='shake_acted')
        sig_op = tf.clip_by_value(tf.nn.sigmoid(shake_acted), 0.001, 0.999, name='shake_clipped_sigmoid')
        sig_loss = tf.to_float(shake_success_batch) * -tf.log(sig_op) + (1 -
                tf.to_float(shake_success_batch)) * -tf.log(1 - sig_op)
        self.sig_op = sig_op
        ...
        conf = tf.equal(tf.to_int32(tf.greater_equal(sig_op,0.5)),tf.to_int32(shake_success_batch))
        self.conf = conf
