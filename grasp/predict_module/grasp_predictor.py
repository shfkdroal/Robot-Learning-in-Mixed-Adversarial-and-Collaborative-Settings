...
class Predictors:
    ...
    def graspNet_grasp(self, num_angle=18, patch_size=300, num_samples=128):
        ...
        for looper in range(num_samples):
            ...
            while isWhiteFlag==1:
                if self.opt == 0:
                    ...
                    patch_hs[looper] = self.update_info[1][looper]
                    patch_ws[looper] = self.update_info[2][looper]

                s = self.I[patch_hs[looper] - half_patch_size:patch_hs[looper] - half_patch_size + patch_size,
                patch_ws[looper] - half_patch_size:patch_ws[looper] - half_patch_size + patch_size].shape

                if s == patch_Is[looper].shape:
                    patch_Is[looper] = (self.I[patch_hs[looper]-half_patch_size:patch_hs[looper]-half_patch_size+patch_size,
                                       patch_ws[looper]-half_patch_size:patch_ws[looper]-half_patch_size+patch_size])
                else:
                    #print("patch_hs[0]: ", patch_hs[looper])
                    #print("patch_ws[0]: ", patch_ws[looper])
                    #print("half_patch_size: ", half_patch_size)
                    #print("pat1:", patch_hs[looper] - half_patch_size)
                    #print("pat2:", patch_hs[looper] - half_patch_size + patch_size)
                    #print("patch_Is[looper] shape: ", patch_Is[looper].shape)
                    #print("i_temp shape: ", s)
                    #print("I shape: ", self.I.shape)
                    while not(s == patch_Is[looper].shape):
                        patch_hs[looper] = np.random.randint(h_range) + half_patch_size
                        patch_ws[looper] = np.random.randint(w_range) + half_patch_size
                        patch_Is[looper] = (self.I[patch_hs[looper]-half_patch_size:patch_hs[looper]-half_patch_size+patch_size,
                                       patch_ws[looper]-half_patch_size:patch_ws[looper]-half_patch_size+patch_size])
        ...
        self.r_angle_table_patches = np.zeros([num_samples], dtype=object)
