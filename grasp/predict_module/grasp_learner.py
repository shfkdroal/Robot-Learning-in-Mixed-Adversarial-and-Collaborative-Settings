...
class grasp_obj:
    ...
    def test_init(self, lr_rate):
        ...
        # training op, Jiali
        with tf.device(self.dev_name):
            self.pred = tf.reduce_max(self.fc8_norm_vals, axis=1, keepdims=True) # [None, 1] -> [128 == None, 1] (shelf.y.shape == [1])
            self.probs  = tf.sigmoid(self.pred)
            self.loss_vec = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.pred, labels=self.y) # [128, 1]
            self.loss = tf.reduce_mean(self.loss_vec) #sum of loss_vec/128
